...
def update_data(data, priority=True):
    db = Database()
    table_order = ['tracks', 'races','horses', 'horse_races']
    table_map = {'tracks': 'races_track', 'races': 'races_race', 
                 'horses':'horses_horse', 'horse_races': 'horses_horserace'}
    for table in table_order:
        ...
        existing = {d[pk]: d for d in db.execute(f"SELECT * FROM {db_table}", as_dict=True)}
        to_create = {}
        for d in data[table]:
            obj = existing.get(d[pk])
            tc_obj = to_create.get(d[pk])
            if obj:
                for k,v in d.items():
                ...
                to_create[d[pk]] = d
        if to_create:
            db.insert_df(pd.DataFrame(to_create.values()), db_table)
        if existing:
            df = pd.DataFrame(existing.values())
            if priority:
                db.upsert_df_except_null(df, db_table)
            else:
                db.upsert_df_only_null(df, db_table)
